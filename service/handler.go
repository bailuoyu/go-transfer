...
func (s *handler) OnRow(e *canal.RowsEvent) error {
	//canalBody, _ := json.Marshal(e)
	//logs.PrintInfof("canal info :%s", canalBody)
	...
	//logs.PrintInfof("row reqs <- :%s", body)
	s.queue <- requests
		...
		for {
			...
			case v := <-s.queue:
				...
				case []*model.RowRequest:
					//requests = append(requests, v...)
					for _, sv := range v {
						sv.LogName = current.Name
						requests = append(requests, sv)
					}
					//body, _ := json.Marshal(requests)
					//logs.PrintInfof("listen reqs -> :%s", body)
			...
			if needSavePos && _transferService.endpointEnable.Load() {
				...
				logs.PrintInfof("save position %s %d", current.Name, current.Pos)
				if err := _transferService.positionDao.Save(current); err != nil {
					//logs.Errorf("save sync position %s err %v, close sync", current, err)
					logs.PrintErrorf("save sync position %s err %v, close sync", current, err)
