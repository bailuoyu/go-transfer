...
import (
	...
	"github.com/bailuoyu/go-transfer/storage"
...
func (s *TransferService) initialize() error {
	...
	logs.PrintInfof("transfer dump exec: %s", global.Cfg().DumpExec)
...
func (s *TransferService) run() error {
	...
	var err error
	//如果是强制指定位置
	if global.Cfg().InitLogForce && global.Cfg().InitLogFile != "" {
		current = mysql.Position{
			Name: global.Cfg().InitLogFile,
			Pos:  global.Cfg().InitLogPos,
		}
	} else {
	...
	go func(p mysql.Position) {
		...
		log.Println(fmt.Sprintf("transfer run from position(%s %d)", current.Name, current.Pos))
