...
import (
	...
	"github.com/bailuoyu/go-transfer/util/files"
	"github.com/bailuoyu/go-transfer/util/logs"
	"github.com/bailuoyu/go-transfer/util/nets"
	"github.com/bailuoyu/go-transfer/util/sys"
)
...
type Config struct {
	...
	SkipMasterData bool   `yaml:"skip_master_data"` //跳过主表全数据检查，开启之后初始化数据会锁表
	InitLogFile    string `yaml:"init_log_file"`    //初始化日志文件，初次执行1236错误，宕机，缓存日志丢失
	InitLogPos     uint32 `yaml:"init_log_pos"`     //初始化日志位置，初次执行1236错误，宕机，缓存日志丢失
	InitLogForce   bool   `yaml:"init_log_force"`   //是否强制从指定位置开始，仅用于纠正错误时特殊执行
	...
	KafkaVersion      int    `yaml:"kafka_version"`       //kafka版本，1-2，默认2
	...
	KafkaSASLType     string `yaml:"kafka_sasl_type"`     //kafka SASL_PLAINTEXT类型，默认SCRAM-SHA-512
